	...
	void Update()
	{
		if(!sequenceStarted && endSkull.position.z < -7.93f)
		{
			...
			Waiters.Wait(3f)
				   ...
				   .ThenInterpolate(0.5f, f => 
					{
						endTextFade2.material.SetColor("_TintColor", Color.Lerp(Color.white, Color.clear, Easing.EaseOut(f, EaseType.Quad)));
					});
